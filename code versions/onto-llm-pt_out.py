...
async def update_metadata_from_descriptions_cache(
    ...
    in_prompt = f"Give a brief description of every term on the list, considered as a subcategory of {concept}, without the use of examples, in Brazilian Portuguese and in the following form:\nList element 1: brief description of list element 1.\nList element 2: brief description of list element 2.\n..."
    prompt_message = {"role": "user", "content": in_prompt}
    messages = definition_messages([initial_concept, concept], definitions)
    messages.append(
        {
            "role": "user",
            "content": f'List all of the most important subcategories of "{concept}" in a comma-separated format, skip explanations. Answer in Brazilian Portuguese',
        }
    )
    messages.append(
...
def hierarchy2dot(hierarchy: dict[str, dict[str, Any]]) -> str:
    ...
    metadata_label = (
        "Concepts: {}\\l Subsumptions: {}\\l Exploration Depth: {}\\l Frequency Threshold: {}\\l PT-BR Output\\l Chosen Model: {}\\l"
    ).format(
...
async def concept_definition_initial(client, initial_concept: str):
    ...
    in_prompt = f'Give a very brief description of the term "{initial_concept}" without the use of examples. Answer in Brazilian Portuguese.'
    prompt_message = {"role": "user", "content": in_prompt}
...
async def subconcepts_definitions(
    ...
    if to_fetch:
        ...
        messages.append(
            {
                "role": "user",
                "content": f'List all of the most important subcategories of "{concept}" in Brazilian Portuguese and in a comma-separated format, skip explanations.',
            }
        )
        messages.append(
...
async def create_completion_sample(
    ...
    if concept == bottom_concept:
        return []
    if concept == initial_concept:
        in_prompt = f'List all of the most important subcategories of "{concept}". Answer in Brazilian Portuguese, skip explanations and use a comma-separated format like this:\nImportant subcategory, Another important subcategory, Another important subcategory, etc.'
    elif parents == [initial_concept]:
        in_prompt = f'"{concept}" is a subcategory of "{initial_concept}". List all of the most important subcategories of "{concept}". Answer in Brazilian Portuguese, skip explanations and use a comma-separated format like this:\nImportant subcategory, Another important subcategory, Another important subcategory, etc.'
    else:
        in_prompt = f'"{parents[-1]}" is a subcategory of "{initial_concept}". "{concept}" is a subcategory of "{parents[-1]}". List all of the most important subcategories of "{concept}". Answer in Brazilian Portuguese, skip explanations and use a comma-separated format like this:\nImportant subcategory, Another important subcategory, Another important subcategory, etc.'
    prompt_message = {"role": "user", "content": in_prompt}
...
async def rename_subconcept(
    ...
    if concept != initial_concept:
        in_prompt = f'"{concept}" is a subcategory of "{initial_concept}". The following description outlines the characteristics of a subcategory of "{concept}". Provide a concise and unambiguous name for it. Provide only the name in Brazilian Portuguese and without any explanation. Description: {definitions[subconcept.name]}'
    else:
        in_prompt = f'The following description outlines the characteristics of a subcategory of "{concept}". Provide a concise and unambiguous name for it. Provide only the name in Brazilian Portuguese and without any explanation. Description: {definitions[subconcept.name]}'
    prompt_message = {"role": "user", "content": in_prompt}
    messages = definition_messages([initial_concept, concept], definitions)
    messages.append(prompt_message)
...
def main():
    ...
    async def main_async():
        async with httpx.AsyncClient(timeout=timeout) as client:
            await kris_alg(client, hierarchy) #2816
        output_hierarchy_and_metadata(hierarchy, filepath) #2885

    try:
        asyncio.run(main_async()) #2873
